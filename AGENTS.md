...
2. **MLP + reranker (deployable pipeline)** — an MLP classifier scores antonymy from axis-wise
 ...
 (MLP top-100 ∪ predicted-axis top-20 ∪ procrustes top-20 ∪ ICA-map top-20
 ∪ morphological prefix transforms) down to top-10.
...
|`src/reranker.py`|Logistic reranker (14 signals) + CandidateSources (5-source union pool incl. morphological) + CV|
...
Retrieval eval (scripts/eval_reranker.py):
 ...
 (MLP top-100 ∪ knn-axis top-20 ∪ procrustes top-20 ∪ ICA-map top-20 ∪ morph)
 -> logistic reranker (14 signals incl. source flags) -> Hits@{1,5,10}
```
