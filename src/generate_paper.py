...
def generate_paper(results_dir: str = "results", output_path: str = "paper/paper.md") -> None:
    ...
    w("### Reranker")
    w()
    w("Five complementary retrievers build a union candidate pool:")
    ...
    w("vector toward its antonym), an ICA-score translation map's top-20")
    w("(W_ica maps s(src) toward s(tgt) in score space), and morphological")
    w("prefix transforms. A logistic reranker")
    w("re-orders the union down to a top-10 list using fourteen signals:")
    w()
    ...
    w("| in_morph | Candidate came from morphological prefix transforms |")
    w()
    ...
    w()
    w("Morphological candidates (un-/in-/dis-/... prefix transforms) add a")
    w("fifth source; they need no model and cover ~33% of WordNet pairs.")
    w()
